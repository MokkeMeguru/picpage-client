...
* [`src/clj/picpage_client/`](src/clj/picpage_client/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/picpage_client/`](src/cljs/picpage_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/picpage_client/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/picpage_client/`](test/cljs/picpage_client/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `picpage-client` project root directory
...
in the [`src/clj/picpage_client/`](src/clj/picpage_client/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/picpage_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns picpage-client.example
  (:require [picpage-client.config :as config])
